...
dfavg = df1.groupby('race/ethnicity', as_index=False)['average score'].mean()
bar_chart = px.bar(dfavg, x="race/ethnicity", y="average score", title="Average Score each group")
st.plotly_chart(bar_chart)

dfavgmath = df1.groupby('race/ethnicity', as_index=False)['math score'].mean()
...
dfavgreading = df1.groupby('race/ethnicity', as_index=False)['reading score'].mean()
...
dfavgwriting = df1.groupby('race/ethnicity', as_index=False)['writing score'].mean()
